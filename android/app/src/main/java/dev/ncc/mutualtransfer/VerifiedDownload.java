...
/** Bounded streaming copy with exact length and SHA-256 verification. */
public final class VerifiedDownload {
    ...
    public static long copy(InputStream input, OutputStream output, String expected, long size, BooleanSupplier cancelled) throws IOException {
        return copy(input, output, expected, size, MAX_BYTES, cancelled);
    }
    static long copy(InputStream input, OutputStream output, String expected, long size, long limit, BooleanSupplier cancelled) throws IOException {
        if (expected == null || !expected.matches("[a-f0-9]{64}") || size < -1 || limit < 0 || limit > MAX_BYTES || size > limit) throw new IOException("Invalid download metadata or staging limit");
        ...
        while (true) {
            ...
            if (read == -1) break;
            if (cancelled.getAsBoolean() || count + read > limit || (size >= 0 && count + read > size)) throw new IOException("Cancelled or oversized response");
