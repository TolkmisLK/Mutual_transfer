...
This addresses a distinct source-review finding, not the still-unexplained intermittent instrumentation crash. Main run `35352777102` reported a Java `IllegalStateException` without an application frame; privacy-filtered diagnostics now also retain allowlisted framework frames if it recurs. Two later candidate Android runs (`35381520810`, `35381670016`) passed; that does not establish the crash root cause. No process-death, indefinite-provider-blocking, rotation or background guarantee follows from the controlled queue test.
