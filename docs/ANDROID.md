...
The next candidate additionally creates a unique 65,537-byte Downloads-provider fixture, selects it through the actual system file picker, and saves a server download through the actual system create-document UI. It checks the production Activity's completion status and independently reads the exact new destination bytes, then cleans up only those generated files. This system-provider test is pending CI; no result is claimed yet. Other providers, permission revocation, cancellation, rotation and physical devices remain separate checks.
