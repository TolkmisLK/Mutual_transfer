...
Windows CI acceptance is pending for this packaging slice. Until recorded in [VALIDATION.md](VALIDATION.md), do not treat the presence of a packaging script as a validated downloadable build. Consumer clean-machine startup, browser association, physical LAN transfer and OS security prompts still need real-machine acceptance; do not bypass SmartScreen or certificate warnings.
